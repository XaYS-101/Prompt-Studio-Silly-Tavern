...
### Version history
A snapshot of the prompt is taken automatically before each editing session, and manually whenever you want (with a note). The History tab shows all snapshots of the current preset, filterable by prompt, with a colour word-level **diff against the current text**, restore and delete. Restoring first snapshots the current state, so it's always reversible. Limits (per prompt / total / max size), deduplication and a storage indicator keep `settings.json` from bloating.
